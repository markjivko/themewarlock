<?php
/**
 * Theme Warlock - Addon_CoreVcCallToAction
 * 
 * @title      WPBakery Page Builder - Call-to-Action
 * @desc       Grab the visitor's attention with a simple call-to-action ribbon
...
class Addon_CoreVcCallToAction extends Addon {
    ...
    const KEY_DEF_SECT_BUTTON      = 'defSectButton';
     ...
     * @var string
     */
    public static $addonIcon = Twitter_Bootstrap_GlyphIcon::GLYPH_CERTIFICATE;
    ...
    public static function getOptions() {
        return array(
            ...
            (new Model_Project_Config_Item_String(self::KEY_DESCRIPTION))
                ->setValue('A simple plugin that creates a customizable call-to-action ribbon')
                ->setIsTextarea(true)
            ...
            (new Model_Project_Config_Item_String(self::KEY_DEF_SECT_SUBTITLE))
                ->setValue('Do not hesitate, buy now!')
                ->setMetaTitle('Default VC section subtitle')
                ->setMetaDescription('Set the default subtitle in the VC add-on'),
            
            (new Model_Project_Config_Item_String(self::KEY_DEF_SECT_BUTTON))
                ->setValue('Buy now!')
                ->setMetaTitle('Default VC section button text')
                ->setMetaDescription('Set the default button text in the VC add-on'),
    ...
    public function getPlugins() {
        return array(
            Plugin::PLUGIN_VC_CALL_TO_ACTION,
        );
    }
    ...
    public function onPluginDeployment(Drawables_Plugin_Common $pluginCommon) {
        switch ($pluginCommon->getPlugin()->getName()) {
            case Plugin::PLUGIN_VC_CALL_TO_ACTION:
